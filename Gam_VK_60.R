...
Kel_ap_w=window(Kel_ap,start(BVP_w),end(BVP_w))
Kro_ap_w=window(Kro_ap,start(BVP_w),end(BVP_w))

H_PP_w=window(H_PP,start(BVP_w),end(BVP_w))
I55_PP_w=window(I55_PP,start(BVP_w),end(BVP_w))
H_I_PP_W=H_PP_w+I55_PP_w
J_PP_w=window(J_PP,start(BVP_w),end(BVP_w))
L68_PP_w=window(L68_PP,start(BVP_w),end(BVP_w))
M_N_PP_w=window(M_N_PP,start(BVP_w),end(BVP_w))
N_PP_w=window(N_PP,start(BVP_w),end(BVP_w))
P_PP_w=window(P_PP,start(BVP_w),end(BVP_w))
Q_PP_w=window(Q_PP,start(BVP_w),end(BVP_w))
P_Q_PP_w=P_PP_w+Q_PP_w
R_PP_w=window(R_PP,start(BVP_w),end(BVP_w))
S_PP_w=window(S_PP,start(BVP_w),end(BVP_w))
R_S_PP_w=R_PP_w+S_PP_w
...
mod_F=lm(F_w~SD_w+DS_F_l1+S4_w)
summary(mod_F)
e_F=resid(mod_F)
shapiro.test(e_F)
...
## prognozė
Ff=predict(mod_F, 
           list(SD_w=c(window(SD, start=c(2011,1), end=c(now))),
                DS_F_l1=c(window(lag(DS_F,-1), start=c(2011,1), end=c(now))),
                S4_w=c(window(S4, start=c(2011,1), end=c(now)))), interval = "confidence")
...
mod_G_I=lm(G_I_w~G_ap_w+S1_w+S1_w+S3_w)
summary(mod_G_I)
e_G_I=resid(mod_G_I)
shapiro.test(e_G_I)
...
G_If=predict(mod_G_I, 
             list(G_ap_w=c(window(G_ap, start=c(2011,1), end=c(now))),
                  S1_w=c(window(S1, start=c(2011,1), end=c(now))),
                  S3_w=c(window(S3, start=c(2011,1), end=c(now))),
                  H_PP_w=c(window(H_PP, start=c(2011,1), end=c(now)))), interval = "confidence")
...
mod_L=lm(L_w~SD_l3+SD_l4+SL_NP_l3+L68_PP_w)
summary(mod_L)
e_L=resid(mod_L)
shapiro.test(e_L)
...
## prognozė
Lf=predict(mod_L, 
           list(L_l1=c(window(lag(L,-1), start=c(2011,1), end=c(now))),
                SD_l3=c(window(lag(SD,-3), start=c(2011,1), end=c(now))),
                SD_l4=c(window(lag(SD,-4), start=c(2011,1), end=c(now))),
                SL_NP_l3=c(window(lag(SL_NP,-3), start=c(2011,1), end=c(now))),
                L68_PP_w=c(window(L68_PP, start=c(2011,1), end=c(now)))), interval = "confidence")
...
M_Nf=predict(mod_M_N, 
             list(M_N_PP_w=c(window(M_N_PP, start=c(2011,1), end=c(now)))), interval = "confidence")
...
mod_R_T=lm(R_T_w~R_S_PP_w+S3_w)
summary(mod_R_T)
e_R_T=resid(mod_R_T)
shapiro.test(e_R_T)
...
R_Tf=predict(mod_R_T, 
             list(R_S_PP_w=c(window(R_S_PP, start=c(2011,1), end=c(now))),
                  S3_w=c(window(S3, start=c(2011,1), end=c(now)))), interval = "confidence")
...
BVP_VK_60=data.frame(Af, B_Ef, Cf, Ff, G_Tf, G_If, Jf, Kf, Lf, M_Nf, O_Qf, R_Tf,A_Tf, D21f, D31lef, D21_D31f, BVPf, 
                  stringsAsFactors = FALSE)
names(BVP_VK_60)=c("A", "B+C+D+E","C","F","G+...+T", "G+H+I", "J", "K","L","M+N", "O+P+Q", "R+S+T","A+...+T",
                "D21", "D31", "D21-D31","BVP")
plot(BVP_w,lwd=2)
lines(BVPf,col="red")

write.xlsx2(BVP_VK_60, file="BVP_prognoze_GVK_60.xlsx", sheetName = "Gamybos_VK_60",
